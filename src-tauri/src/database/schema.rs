...
impl Database {
        ...
        let result = (|| {
            while version < SCHEMA_VERSION {
                match version {
                    ...
                    16 => {
                        log::info!("迁移数据库从 v16 到 v17（协调 Codex 用量与额度历史，并添加会话用量持久去重账本）");
    ...
    fn migrate_v16_to_v17(conn: &Connection) -> Result<(), AppError> {
        // Both the local fork and upstream shipped different schema-v16
        // lineages. Do not rerun the destructive v15 -> v16 Codex rebuild
        // here: upstream v16 databases already completed it, and a second
        // reset could discard history whose source JSONL no longer exists.
        // The missing v17 structures can be reconciled non-destructively.
        Self::ensure_codex_quota_history_table(conn)?;
        Self::ensure_session_usage_dedup_table(conn)
    }
    ...
    /// Preserve session request identities after detail rollup.
    fn ensure_session_usage_dedup_table(conn: &Connection) -> Result<(), AppError> {
        conn.execute_batch(
            "CREATE TABLE IF NOT EXISTS session_usage_dedup (
                data_source TEXT NOT NULL,
                request_id TEXT NOT NULL,
...
mod tests {
    ...
    #[test]
    fn migrate_v16_to_v17_reconciles_schema_without_resetting_codex_usage() -> Result<(), AppError>
    {
        let conn = Connection::open_in_memory()?;
        Database::create_tables_on_conn(&conn)?;
        ...
        assert_eq!(codex_rows, 1, "v16 -> v17 must not erase rebuilt history");
        Ok(())
    }
